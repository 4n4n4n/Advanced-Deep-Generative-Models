...
class VAE(nn.Module):
    ...
    def sample_with_reparametrization(self, mu: TensorType['batch_size', 'latent_dim'], 
        ...
        ##########################################################
        # YOUR CODE HERE
        # since we assume the mean field assumption, the j-th component of the i-sample only dependent on (z_i)_j which is a simple Gaussian
        # so we can directly apply torch.normal whcih perform entrywise normal 
        # we need reparametrization

        # reparametrize
        samples = torch.normal(torch.zeros(mu.size()), torch.ones(mu.size()))
        sigma = torch.exp(logsigma) 
        samples = sigma * samples + mu
        return samples
    ...
    def kl_divergence(self, mu: TensorType['batch_size', 'latent_dim'], logsigma: TensorType['batch_size', 'latent_dim']) -> TensorType['batch_size']:
        ...
        kl = 0.5 * torch.sum(mu.pow(2) + var - torch.log(var)-1, 1)
        return kl
    ...
    def elbo(self, x: TensorType['batch_size', 'input_dim']) -> TensorType['batch_size']:
        ...
        kl = self.kl_divergence(mu, logsigma) # batch
        theta = self.decoder(z) # shape = [batch, input_dim]
        
        # log_two_pi = torch.log(
        #                 torch.tensor(2.0 * torch.pi, device=x.device, dtype=x.dtype))  
        # D = x.size(1)
        # ls = -0.5 *( torch.sum((x-theta).pow(2) , 1)+D*log_two_pi)  # [batch]
        ls = -F.binary_cross_entropy(theta, x, reduction = 'none').sum(1) # BCE will automatically make it negative
        elbo_mc = ls -kl 
        return elbo_mc
        ##########################################################
    ...
    def sample(self, num_samples: int, device: str='cpu') -> Tuple[
        ...
        ##########################################################
        # YOUR CODE HERE
        z = torch.normal(torch.zeros((num_samples, self.latent_dim)), torch.ones((num_samples, self.latent_dim))) 
        theta = self.decoder(z) # [num_samples, inptu_dim]
